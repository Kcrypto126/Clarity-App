...
-- Create user_intro_assessments table
CREATE TABLE public.user_intro_assessments (
    id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id uuid REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
    completed_at timestamptz,
    assessment_version text NOT NULL,
    unlocked_nodes text[] DEFAULT '{}'::text[],
    created_at timestamptz DEFAULT now() NOT NULL,
    updated_at timestamptz DEFAULT now() NOT NULL,
    UNIQUE(user_id)
);

-- Create RLS policies for user_intro_assessments table
ALTER TABLE public.user_intro_assessments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users can read their own intro assessment" ON public.user_intro_assessments
    FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own intro assessment" ON public.user_intro_assessments
    FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own intro assessment" ON public.user_intro_assessments
    FOR UPDATE USING (auth.uid() = user_id);

-- Create indexes for better query performance
CREATE INDEX idx_user_node_states_user_id_node_id ON public.user_node_states(user_id, node_id);
CREATE INDEX idx_user_responses_user_id_node_question ON public.user_responses(user_id, node_id, question_id);
CREATE INDEX idx_journal_entries_user_id_created ON public.journal_entries(user_id, created_at DESC);
CREATE INDEX idx_user_intro_assessments_user_id ON public.user_intro_assessments(user_id);
...
CREATE TRIGGER user_intro_assessments_updated_at
    BEFORE UPDATE ON public.user_intro_assessments
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_updated_at();
